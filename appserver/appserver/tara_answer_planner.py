...
_HUNDRED_YEAR_PATTERN_QUESTIONS = (
    ...
    re.compile(r"\b(?:load|show|open|explain|analy[sz]e|tell me about|what is|what's)\b.{0,35}\b(?:pattern from|pattern in) (?:the|my|your|afshin(?:'s)?) book\b", re.I),
    re.compile(r"\b(?:the\s+)?pattern (?:from|in) (?:the|my|your|afshin(?:'s)?) book\b", re.I),
...
def is_hundred_year_pattern_question(message: Any) -> bool:
    ...
    text = str(message or "").strip()
    return bool(text and any(pattern.search(text) for pattern in _HUNDRED_YEAR_PATTERN_QUESTIONS))
...
def build_hundred_year_pattern_command(
    ...
    *,
    ...
    if occurrence_status == "upcoming":
        ...
        occurrence_line = (
            f"{occurrence_start.year} is upcoming - it starts {occurrence_start.strftime('%b')} "
            f"{occurrence_start.day}, {occurrence_start.year} in {days_until} calendar days and "
            f"ends {occurrence_end.strftime('%b')} {occurrence_end.day}, {occurrence_end.year}. "
            f"Its empty row is shown separately and excluded from the completed n={completed_count}."
        )
        ...
        occurrence_line = (
            ...
            f"{HUNDRED_YEAR_DISPLAY_DAYS}. Its partial row is shown separately and excluded "
            f"from the completed n={completed_count} until the window ends "
            f"{occurrence_end.strftime('%b')} {occurrence_end.day}, {occurrence_end.year}."
        )
    else:
        occurrence_line = (
            f"The {occurrence_start.year} window ended "
            f"{occurrence_end.strftime('%b')} {occurrence_end.day}, {occurrence_end.year}; "
            f"the viewer now includes it in the completed n={completed_count}."
        )
    ...
    if _ADVICE_PATTERNS.search(str(message or "")):
        lines.append(
            "<div class=\"tara-analysis-section tara-analysis-scope\">Past performance and model "
            "estimates do not guarantee future results. TradeWave provides research context, not "
            "individualized recommendations.</div>"
        )
    return {"spec": spec, "reply": "<div class=\"tara-analysis\">" + "".join(lines) + "</div>"}
    ...
    *,
    ...
    for item in yearly_results:
        ...
        try:
            ...
            continue
        underlying_return = _number(
            item.get("underlying_return_pct", item.get("return_pct"))
        )
...
def _analysis_ai_context_line(
    ...
    if context["mode"] == "checkpoints":
        ...
        if len(probabilities) >= 2 and len(predicted_returns) >= 2:
            best_probability = max(value for _, value in probabilities)
            best_return = max(value for _, value in predicted_returns)
            best_probability_days = [
                day for day, value in probabilities if value == best_probability
            ]
            best_return_days = [
                day for day, value in predicted_returns if value == best_return
            ]
...
def _analysis_payoff_and_path_line(facts: Mapping[str, Any]) -> Optional[str]:
    ...
    if wins >= 2 and losses >= 2 and avg_win is not None and avg_loss is not None:
        ...
        if losses == 1:
            details.append(
                "with only one losing observation, the winner/loser payoff ratio is not stable enough to interpret"
            )
        elif wins == 1:
            details.append(
                "with only one profitable observation, the winner/loser payoff ratio is not stable enough to interpret"
            )
        else:
            details.append(
                "the winner/loser payoff comparison is based on too few outcomes to be stable"
            )
    ...
    if (
        ...
        details.append(
            f"the only losing observation ({only_losing_year}) still reached "
        ...
        details.append(
            "losing observations still reached median MFE "
            + _pct(facts["median_losing_mfe_pct"], signed=True, decimals=2)
            + " before their losing finishes"
        )
...
def _analysis_compact_context_line(
    ...
    if avg_return is not None and avg_return > 0 and avg_without_best is not None and avg_without_best <= 0:
        ...
        prior_wins = int(facts.get("prior_profitable_years") or 0)
        issue = (
            f"recent record: {recent_wins} of {recent_n} profitable versus {prior_wins} of the prior "
            f"{prior_n}; this small slice does not establish lasting deterioration"
        )
...
def verified_context_lines(
    ...
    if facts.get("tradewave_ratio") is not None:
        ...
        if facts.get("sharpe_ratio") is not None:
            comparison = (
                f"; Sharpe {facts['sharpe_ratio']:.2f} uses the final returns"
            )
        lines.append(
            f"- TradeWave Ratio (TWR) {facts['tradewave_ratio']:.2f} applies the Sharpe-style "
            f"return-to-dispersion calculation to each year's MFE instead of its final return"
            f"{comparison}."
        )
    path_facts = []
    if facts.get("median_mfe_pct") is not None:
        path_facts.append(
            "median MFE " + _pct(facts["median_mfe_pct"], signed=True, decimals=2)
        )
    if facts.get("median_mae_pct") is not None:
        path_facts.append(
            "median MAE " + _pct(facts["median_mae_pct"], signed=True, decimals=2)
        )
    if facts.get("worst_mae_pct") is not None:
        worst_mae = "worst MAE " + _pct(
            facts["worst_mae_pct"], signed=True, decimals=2
        )
        if facts.get("worst_mae_year") is not None:
            worst_mae += f" in {facts['worst_mae_year']}"
        path_facts.append(worst_mae)
    if path_facts:
        lines.append("- Entry-relative path: " + "; ".join(path_facts) + ".")
    ...
    if (
        ...
        lines.append(
            f"- IMPORTANT ENDPOINT-VERSUS-PATH FACT: the only losing observation, "
            f"{facts['only_losing_year']}, reached "
            f"{_pct(facts['only_losing_mfe_pct'], signed=True, decimals=2)} MFE before finishing "
            f"{_pct(facts['only_losing_return_pct'], signed=True, decimals=2)}; that is a "
        ...
        lines.append(
            "- Losing-finish path: the losing observations still reached median MFE "
            + _pct(facts["median_losing_mfe_pct"], signed=True, decimals=2)
    ...
    if (
        ...
        lines.append(
            f"- Recent slice: {facts.get('recent_profitable_years', 0)} of "
            f"{facts['recent_sample_size']} profitable versus "
            f"{facts.get('prior_profitable_years', 0)} of {facts['prior_sample_size']} prior; "
            f"n={facts['recent_sample_size']} is too small by itself to establish lasting deterioration."
        )
