# this script first loads all the outstanding orders that are valid order
# it also loads all the positions that are active
# then it checks all the orders stored in orders fields for each of the opportunities against valid orders list
# if order did not exist in the valid order list and it didn't have a corresponding position, then order number is deleted from redis
...
def remove_canceled_orders(order_list):
    ...
    for order_id in order_list:
        ...
        if order_dict['order']['status'] == 'expired' or order_dict['order']['status'] == 'rejected' or order_dict['order']['status'] == 'canceled':
