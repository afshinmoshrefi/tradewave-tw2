...
const PopulatePortfolio = (props) => {
    ...
    const [minSR2, SetMinSR2] = useState('1.0');

    const [avgRet, SetAvgRet] = useState('3.0%');
    const [avgRet2, SetAvgRet2] = useState('4.0%');
    ...
    const daysSelectionList = () =>{
        let tmp = [];
        for (let i = 0; i <= 20; i++) {
            tmp.push({ id: i + 1, value: i, label: i });
        }
    ...
    const [keepHighestSR, SetKeepHighestSR] = useState(false);
    ...
    var cancel_width = '18%';
    var trash_blue_square_width = '30%'
    


    var populate_width = '40%';
    ...
    if (rdd.isMobile && !rdd.isTablet && browserH > browserW) {
        ...
        buttonTextFont = '2.9vw';
        trash_blue_square_width = '50%'
        text_font_size = '3.8em';
    }
    ...
    useEffect(() => { // get default cookie
        ...
        if (qvars !== null) {
            ...
            if (params[12] !== 'undefined') SetMinSR2(params[12]);
            if (params[13] !== 'undefined') SetAvgRet2(params[13] + '%');
            if (params[14] !== 'undefined'){
                if (params[14] === '0')SetKeepHighestSR(false);
                else                   SetKeepHighestSR(true);
            }
            if (params[15] !== 'undefined') SetSelectedDays(parseInt(params[15]));
        }
    ...
    const handleBlur = (event) => {
        ...
        switch (event.target.id) {
            ...
            case 'min_SR2':
                if (!isNaN(Number(tmp))) {
                    SetTextBoxError1(false);
                    SetMinSR2(Number(tmp).toFixed(1));
                } else {
                    SetTextBoxError1(true);
                }
                break;
            case 'min_avg_return':
                ...
                break;
            case 'min_avg_return2':
                if (!isNaN(Number(tmp))) {
                    SetTextBoxError2(false);
                    SetAvgRet2(Number(tmp).toFixed(1) + '%');
                } else {
                    SetTextBoxError2(true);
                }
                break;
    ...
    const portfolioActions = (action) => {
        ...
        let fhs = 0 // converts keepHighestSR to binary 0 and 1 instead of true false
        if (keepHighestSR === true)fhs=1
        ...
        // console.log('vars_combined=',vars_combined)
        let qvars = window.btoa(vars_combined);
        ...
        fetch(url)
            ...
            .then((data) => {
                ...
                SetDisplayWorking('none');  // this is for the red "working ..." indicator while the api call is being completertd
                SetOppCount(data['count'])
                
                // props.SetNumReportsCreated(data['count']) // increment the number of reports created

                if (action === 'add' || action === 'remove' || action === 'remove_blue') {
...
//-------------------------------------------------------------------------------------------------------------------------------------
    const checkboxChanged = (event) => {
        SetKeepHighestSR(!keepHighestSR);
    }
    //-------------------------------------------------------------------------------------------------------------------------------------
    return (

        // <div style={DashboardCover} onClick={() => { props.SetShowPopulatePortfolio(false); }}>
        <div style={DashboardCover} >

            <div style={popupDialogStyle} onClick={(e) => { e.stopPropagation(); }}>

                <div style={{ width: '100%', height: '12%', backgroundColor: tc.titleBar, display: 'flex' }}>
                    <div style={{ height: '100%', width: titleCloseDivWidth, backgroundColor: 'transparent' }}></div>
                    <div className='div_basic' style={{ height: '100%', width: titleCenterDivWidth, backgroundColor: 'transparent' }}><span style={{ fontSize: title_text_size, color: tc.textOnControl }}>Populate Portfolio</span></div>
                    <div className='div_basic' onClick={() => { props.SetShowPopulatePortfolio(false); }} style={{ height: '100%', width: titleCloseDivWidth, backgroundColor: tc.closeBtnBg }}><GrClose size={grclose_iconsize} /></div>
                </div>
                ...
                <div className='div_basic' style={{ width: '100%', height: '88%', backgroundColor: 'transparent', flexDirection: 'column' }}>
                    ...
                    <div className='div_basic' style={{ width: '100%', height: '85%', backgroundColor: 'transparent' }}>
                        ...
                        <div style={{ borderRight: '1px solid lightgray', flexDirection: 'column', width: '50%', height: '100%', backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'flex-start' }}>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                <SelectBox optionList={props.securityTypeList} name="securityTypeListP" value={selectedSecurityGroup} sbChanged={selectboxChanged} />
                            </div>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                <SelectBox optionList={monthsOptionsList} value={populateMonth} suffix="" name="monthsP" sbChanged={selectboxChanged} />
                            </div>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                <SelectBox optionList={props.daysOfMonthList} value={populateDay} suffix=" " name="dayP" sbChanged={selectboxChanged} />
                            </div>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                <SelectBox optionList={seasonalYearsOptionsList} value={populateYears} suffix=" years" name="yearsP" sbChanged={selectboxChanged} />
                            </div>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                <SelectBox optionList={partialSeasonalYearsOptionsList} value={populatePartialYears} suffix={` of ${populateYears} years`} name="partialYearsP" sbChanged={selectboxChanged} />
                            </div>
                            <div style={{ width: "80%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'right' }}>
                                {/* <SelectBox optionList={tradeTypes} value={tradeType} suffix="" name="tradeTypes" sbChanged={selectboxChanged} />
                                 */}


                                <div style={{ width: "100%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'right', paddingLeft: '4px' }}>
                                    <span style={{fontSize: text_font_size}}>Initial Status:&nbsp;</span>
                                    {initialStatus === 0
                                        ? <BiSquare size={icon_status_size} style={{ fill: 'gray', verticalAlign: "middle" }} onClick={(event) => handleStatusClicked(event)} />
                                        : <BiSolidSquare size={icon_status_size} style={{ fill: statusColor[initialStatus], verticalAlign: "middle" }} onClick={(event) => handleStatusClicked(event)} />
                                    }
                                </div>
                            </div>
                        ...
                        <div style={{ fontSize: text_font_size, width: '50%', height: '100%', backgroundColor: 'transparent', display: 'flex', flexDirection: 'column', alignItems: "center", justifyContent: 'center' }}>


                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>

                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'right' }}>
                                    <span>Min SR/SR2:</span>
                                </div>
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    <TextBox text={minSR} width={textbox_width} tbBlur={handleBlur} name="min_SR" />
                                    <TextBox text={minSR2} width={textbox_width} tbBlur={handleBlur} name="min_SR2" />
                                    {textBoxError1 && <span style={{ color: 'red', fontSize: redStarSize }}>&nbsp;*</span>}
                                </div>
                            ...
                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>
                                ...
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'right' }}>
                                    <span>AvgR/AvgR2:</span>
                                </div>
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    <TextBox text={avgRet} width={textbox_width} tbBlur={handleBlur} name="min_avg_return" />
                                    <TextBox text={avgRet2} width={textbox_width} tbBlur={handleBlur} name="min_avg_return2" />

                                    {textBoxError2 && <span style={{ color: 'red', fontSize: redStarSize }}>&nbsp;*</span>}
                                </div>
                            ...
                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>
                                ...
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    {/* <TextBox text={minMFE} width={textbox_width} tbBlur={handleBlur} name="min_MFE" /> */}
                                    <SelectBox optionList={minMFEfilters()} value={selectedMinMFE} suffix='%' name="minMFEfilters" sbChanged={selectboxChanged} />
                                    <SelectBox optionList={minMFEpct} value={selectedMinMFEpct} suffix='' name="minMFEpct" sbChanged={selectboxChanged} />
                                </div>
                            </div>
                            ...
                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>
                                ...
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    <TextBox text={dayRange} width={textbox_width} tbBlur={handleBlur} name="day_range" />
                                    {textBoxError4 && <span style={{ color: 'red', fontSize: redStarSize }}>&nbsp;*</span>}
                                </div>
                            </div>
                            ...
                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>
                                ...
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    <TextBox text={minStockPrice} width={textbox_width} tbBlur={handleBlur} name="min_stock_price" />
                                    {textBoxError5 && <span style={{ color: 'red', fontSize: redStarSize }}>&nbsp;*</span>}
                                </div>
                            </div>

                            <div style={{ width: "100%", height: cellHeight, display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'right' }}>
                                    <span>Only Highest SR:</span>
                                </div>
                                <div style={{ width: "50%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'left', paddingLeft: '4px' }}>
                                    <CheckBox name="keep_highest_sr" cbChanged={checkboxChanged} checked={keepHighestSR} />
                                    <SelectBox optionList={daysSelectionList()} name="selectedDays" value={selectedDays} sbChanged={selectboxChanged} />
                                </div>
                            </div>
                            ...
                            <div style={{ width: "100%", height: "20%", display: "flex", backgroundColor: 'transparent', alignItems: "center", justifyContent: 'center' }}>


                                <div style={{ width: "100%", height: "100%", backgroundColor: 'transparent', display: "flex", alignItems: "center", justifyContent: 'center', paddingLeft: '22px' }}>
                                    <button style={{ width: query_width, height: button_height, marginRight: '5%', fontSize: buttonTextFont }} onClick={() => portfolioActions('count')}>Count Opportunities</button>
                                </div>
                    ...
                    <div className='div_basic' style={{ width: '100%', height: '15%', backgroundColor: 'transparent', borderTop: '1px solid ' + tc.border }}>
                        ...
                        <div className='div_basic' style={{ width: '25%', height: '100%', backgroundColor: 'transparent', justifyContent: 'center', fontSize: charIndicatorFontSize }}>
                            <span style={{ color: 'white', backgroundColor: 'red', paddingLeft: '7px', paddingRight: '7px', display: displayWorking }}>working ...</span>
                        </div>

                        <div style={{ display: 'flex', width: '45%', height: '100%', justifyContent: 'center', alignItems: 'center', backgroundColor: 'transparent' }} >

                            <button style={{width: trash_blue_square_width, marginRight: '5%', height: button_height, fontSize: buttonTextFont }} onClick={() => portfolioActions('remove_blue')}><BsTrash3 size={icon_trash_size} style={{  verticalAlign: "middle" }}></BsTrash3> <BiSolidSquare size={icon_status_size} style={{ fill: statusColor['5'], verticalAlign: "middle" }}  /></button>
                            
                            <button style={{ width: cancel_width, marginRight: '5%', height: button_height, fontSize: buttonTextFont }} onClick={() => portfolioActions('remove')}><BsTrash3 size={icon_trash_size} style={{  verticalAlign: "middle" }}></BsTrash3></button>

                            <button style={{ width: populate_width, height: button_height, fontSize: buttonTextFont,marginRight:'2px' }} onClick={() => portfolioActions('add')}>Populate</button>
                        </div>
...
export default PopulatePortfolio 
