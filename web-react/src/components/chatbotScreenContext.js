...
export const buildChatbotScreenContext = (props = {}) => {
  ...
  const priceChartMode = currentPriceChart ? 'current' : (activeTradeChart ? 'active_trade' : 'historical');
  const projectionCapable = currentPriceChart || activeTradeChart;
  const priceChartIsActive = activeIndex === 2;

  const selectedProjectionVisible = priceChartIsActive
    && projectionCapable
    && props.showProjection === true
    && asArrayWithRows(props.consolidatedSeasonalData);

  const selectedLookback = props.seasonalYears == null ? '' : String(props.seasonalYears);
  const fullHistoryYears = props.maxAvailableYears == null ? '' : String(props.maxAvailableYears);
  const fullHistoryProjectionVisible = priceChartIsActive
    && projectionCapable
    && props.showMaxProjection === true
    && asArrayWithRows(props.maxYearsConsolidatedSeasonalData)
    && fullHistoryYears !== ''
    && fullHistoryYears !== '0'
  ...
  const context = {
    ...
    projection_period: props.projectionPeriod == null ? '' : String(props.projectionPeriod),
  ...
  if (!currentPriceChart && lineChartYear !== '') context.price_chart_year = lineChartYear;
  return context;
};
