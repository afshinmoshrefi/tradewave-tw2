...
5. **Regenerate static content** so canonical/og:url/sitemaps carry `tradewave.ai`: re-run the home/scorecard/ticker/insights generators (or trigger the crons). The React build is env-agnostic — **no rebuild**.
