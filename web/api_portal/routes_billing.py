...
def _price_for_tier(tier_name):
    # MONTHLY ONLY (2026-07-05, rationale in apiserver/tiers.py). The cache still
    # buckets by (tier, interval) so a stale annual price in Stripe is simply
    # never resolved rather than crashing the refresh.
    if not _price_cache:
        _refresh_price_cache()
    return _price_cache.get((tier_name, "month"))
...
def billing_checkout():
    ...
    # MONTHLY ONLY. Reject an explicit annual ask loudly rather than silently
    # charging a different interval than the caller requested.
