...
## SMN-SEP25-HANDOFF - owner-directed Claude edition, 2026-09-24T20:03:22Z

Author/session: Codex, current desktop task. Recipient: Claude Code. Status: `awaiting-peer`; the owner directed this one-day handoff. No Claude receipt or acknowledgment is claimed. See daily execution details in [TW-TASK-0005](TW-TASK-0005.md#september25-claude-owned-daily-edition-handoff).

Owner decision: Claude Code, not Codex, owns the September25, 2026 New York-date six-article SMN Dev publication because the owner's ChatGPT allowance is depleted until reset. Use Claude Opus 5 at medium effort for writing and an independent review pass, authenticated through the owner's saved Claude subscription. Do not use Astra, paid API billing, credential copying, or an unverified fallback. This is a temporary one-edition override; it does not settle the longer-term Astra-versus-Claude or cost/quality discussion.

Execution must use the latest SMN `origin/main` and existing runbook/primary `.180` Dev publication gates. Preserve exact date-matched production picks and unchanged TradeWave engine payloads, fresh primary-source context, all existing Dev content/pins/dashboard, heroes, TradeWave charts and links, and both original and added visuals. Keep the owner-confirmed 100% capped chart variant. Do not write to production, send the reader campaign, or change distribution. If a supported Claude subscription job/receipt path is needed, add a small provider-specific adapter or use the documented native Claude Code subscription process; the existing `blog/subscription_writer.py` is Astra-specific (`gpt-6-astra`) and must not be represented as supporting Claude. Maintain separate immutable writer/reviewer evidence and existing source/content/visual/publisher quality gates; reuse successful work and never regenerate to repair packaging.
