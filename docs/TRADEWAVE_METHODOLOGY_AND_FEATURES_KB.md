...
One vertical bar per analyzed year showing that year's return inside the window. **Green = profitable/long-winning year; red = losing/short year; bar height = magnitude.** With MFE/MAE on, each bar splits into a solid segment (realized/MFE) plus a lighter cap (MFE upside above the close) and salmon/pink bars below zero (MAE drawdown). Purpose: "Seasonality is not one number — it is a distribution." It's the fastest way to see whether the edge is broad-based or carried by one **"hero year"** outlier, and whether losing years are small/contained.
...
One bar per historical year inside the window. **Green = profitable year, red = losing year; bar height = magnitude.** Toggle the **`MFE`** and **`MAE`** checkboxes to overlay the path:
...
What to read:
- Consistency: mostly green vs coin-flip.
...
For any loaded card: read the **per-year bar chart** (green = winning/long years, red = losing/short years; solid = MFE, lighter = MAE) for consistency, then the **bottom stats strip** (Sharpe, Avg Gain, % Profitable, Cumulative Ret, **B&H** comparison). Use **Reverse Date Range** to flip a window and find the complementary side to fade or avoid.
