...
One vertical bar per analyzed year showing the underlying security's return inside the window. **Green/up = the underlying rose; red/down = it fell. Profit is direction-aware: green years win for a long setup, while red years win for a short setup.** Bar height = magnitude. With MFE/MAE on, each bar splits into a solid segment (realized/MFE) plus a lighter cap (MFE upside above the close) and salmon/pink bars below zero (MAE drawdown). Purpose: "Seasonality is not one number — it is a distribution." It's the fastest way to see whether the edge is broad-based or carried by one **"hero year"** outlier, and whether losing years are small/contained.
...
- **Days** — window length in CALENDAR days, counting the ENTRY DAY as day 1. The last date in the window is therefore `start + (days - 1)`: Jul 21 with Days=30 ends Aug 19; Jan 1 with Days=366 ends Jan 1 of the next year. Never trading days. (The analytics engine works in `daysOut = days - 1`; the +1 is a display convention so month windows read naturally.) See ecosystem doc §11 (0A).
...
One bar per historical year inside the window. **Green/up means the underlying rose and red/down means it fell; bar height is the underlying move's magnitude. Green wins for a long setup, while red wins for a short setup.** Toggle the **`MFE`** and **`MAE`** checkboxes to overlay the path:
...
What to read:
- Consistency: bars mostly align with the setup direction (green/up for long, red/down for short) rather than looking like a coin flip.
...
For any loaded card: read the **per-year bar chart** (green/up = underlying rose, red/down = it fell; green wins for long and red wins for short; solid = MFE, lighter = MAE) for consistency, then the **bottom stats strip** (Sharpe, Avg Gain, % Profitable, Cumulative Ret, **B&H** comparison). Use **Reverse Date Range** to flip a window and find the complementary side to fade or avoid.
