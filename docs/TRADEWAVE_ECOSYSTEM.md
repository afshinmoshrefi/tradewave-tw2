...
- **Gateway** - `apiserver/` (one letter off the `appserver` data engine, on purpose).
  ...
  (`apiserver/cards.py`) so weak agents render consistently. `PatternCard.setup.exit_date`
  follows invariant 0A: `entry_date + (hold_days - 1)` because the entry is calendar day 1.
...
0A. **DAY COUNTING - a TradeWave "day" is a CALENDAR day, and the entry day is
   day 1 (owner; long-standing, re-confirmed 2026-07-21).** Never trading days,
   never business days. The ANALYTICS ENGINE does not count the entry day: all
   original math uses `daysOut = days - 1`. The DISPLAYED count was later bumped
   +1 purely so month windows read naturally (Jul 1 -> Jul 31 reads "31 days",
   not 30). That bump is COSMETIC and applies to the LABEL ONLY - it must never
   be added to the end date. Therefore, everywhere:
   **`end_date = start + (days - 1)`**. A 30-day window from Jul 21 ends
   **Aug 19** (30 dates inclusive of entry); full July is `start=Jul 1, days=31`
   -> ends **Jul 31**. If you find code doing `start + days`, it is WRONG.
   Reference implementations (SMN): `angle_engine.py` daysOut, `article_images.py`
   `inc_date_day(date1, int(days)-1)`, `angle_chrome.window_end_date()`.
   **Do NOT re-derive this** - it has repeatedly confused both the owner and AI
   models. Bug it caused (SMN, fixed 2026-07-21): `angle_chrome.window_end_date()`
   used `start + days` while the engine and every chart used `days - 1`, so the
   pattern-meta strip said "Aug 20" while chart captions said "Aug 19" inside the
   SAME article; the editorial gate flagged WINDOW_DATE_MISMATCH.
   **When encoding this in a prompt or an LLM reviewer, state the ground truth
   POSITIVELY first** ("TradeWave windows are measured in CALENDAR days"); a bare
   negative ("do not call them trading days") was read BACKWARDS by a reviewer on
   2026-07-21, which asserted the opposite and held a correct article.

0B. **BAR COLOR / SHORT RETURN - `ChartData4[].pct[0]` is the UNDERLYING
   window return, not direction-adjusted trade P&L.** The bar chart draws that
   value directly: green/up means the underlying rose; red/down means it fell.
   Long P&L equals the underlying return, so green years win. Short P&L is the
   inverse, so red/negative years win and green/positive years lose. Any chatbot,
   guide, report, record count, or per-year label must apply the loaded direction
   before calling a year profitable/losing. Keep the raw bar direction visible;
