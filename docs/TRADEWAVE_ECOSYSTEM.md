...
## Shared Work and Bug Memory

[Shared Work Management](WORK_MANAGEMENT.md) defines provider-neutral ownership,
authorization, evidence and handoffs for bugs and features. [Bug Memory](bugs/README.md)
owns outstanding/resolved defect status; [Task Register](tasks/README.md) owns other
substantive work. This ecosystem document remains the architecture/implementation
map. Private agent memories point to these shared records; they are not required
to resume work. The owner wants interchangeable agents with no provider preference.

## 1. The mental model (internalize this)

- **Calculation authority (owner requirement, reaffirmed September 9, 2026):**
  every mathematical result TradeWave already provides must be consumed unchanged
  for the identical study. Never duplicate or correct it in SMN, a renderer,
  model, helper or verifier. Preserve selected years/election phase, dates,
  direction, instrument and engine conventions. A suspected error requires a
  documented TradeWave bug and agreement with Afshin before an engine correction.
  Genuinely new calculations are allowed only after establishing TradeWave does
...
- **Trend interaction audit, September 15, 2026, frontend `fce41885`:**
  ...
  **Right-edge duration (fixed 2026-09-15, main `99bce08d`):** the right resize edge
  ...
  for exact reproductions, source-version references, evidence and acceptance checks.
  Preserve engine values; missing leap-day data cannot be manufactured downstream.
  Existing [label](bugs/TW-BUG-0008.md) and [stable-window](bugs/TW-BUG-0009.md)
  fixes retain their own verification histories. The audit changed no application code.
...
**SMN cross-agent coordination (September22):** SMN root AGENTS.md and CLAUDE.md both route to docs/AGENT_COORDINATION.md; the daily runbook also links it. Canonical claims, peer discussions and handoffs remain in this repository under the existing work-management protocol. TW-TASK-0006 indexes current assignments and efficiency review; TW-TASK-0005 retains edition history. Model/effort disagreements require independent recorded responses and usage/quality evidence, never silent reversal or fabricated consensus. Pending experiments leave publication running on verified settings. Git records are checked at start/resume, checkpoints and before integration; no automatic peer wakeup is installed. Documentation source: SMN 277ba7ddb45d2148186d52d4d5f3761b8ca9c3e3; live application unchanged.

**SMN resilient publication (September22):** Valid 64-hex SHA256 is compared case-insensitively; changed bytes and malformed hashes fail. Package reviews preflight before directory creation. Only empty unrecorded real package directories are reused; nonempty/staged artifacts remain immutable. The owner authorizes bounded autonomous operational recovery, using successful model receipts and preserving failed evidence. Canonical completion receipts take priority over historical holds. No bypass of TradeWave authority, actual review, Dev guards or source custody. TW-TASK-0005 owns September22 recovery evidence.

**Dev tunnel ordering invariant (September24 outage):** On .176, `/etc/cloudflared/config.yml` serves the SMN and other Dev hostnames. Every named ingress route, including ssh-dev, must precede the final `service: http_status:404` catch-all. Appending a route after it prevents cloudflared startup and produces public530 even when both SMN origins are healthy. Validate a candidate with `cloudflared tunnel --config <candidate> ingress validate` before installation/restart; preserve existing routes. TW-TASK-0005 records the repaired incident and browser proof.

**SMN selectable script controller, September26, 2026 (TW-TASK-0009).** SMN main `b559a86483cb89adcc452b69cd4703626132b1ac` adds named Claude and ChatGPT role profiles. `smn_daily_control.py` stores the next-edition selection in `/var/lib/tradewave/smn-daily/settings.json`, pins incomplete editions to their saved roles, locks overlapping runs and records `last-run.json`. On .180, `/usr/local/bin/smn-daily profile chatgpt|claude` selects the provider; `smn-daily run` follows `/opt/smn-daily/current` into the flask-owned versioned source. ChatGPT uses Astra high / Sol medium / Luna low; Claude retains Opus medium / Sonnet low / Haiku low. Fresh primary-source discovery is the only web-enabled model stage; code validates/captures the pages and binds them to research citations. Writing, review and visual checks receive prepared evidence. A single budget counts all stages/retries. Source/publication gates, engine authority and Dev-only targets remain unchanged. The script is installed and focused-verified; **scheduler cutover remains pending server ChatGPT reauthentication**, and the older Windows heartbeat has not been changed. Full state and verification belong to TW-TASK-0009.
**Current SMN daily target, September24, 2026.** Primary Dev `.180` serves `/var/www/smn` and Claude's publishing dashboard (TW-TASK-0008). The `.176` nginx vhost proxies the root to `.180`, while retaining its older `/editions/` files before fallback to primary. Daily automation now uses `subscription_primary_publish.py` from local `smn-primary-dev-publish-20260924`, saved ChatGPT Astra xhigh and existing immutable edition roots. Publisher main `2d1d9e5454026a292ff2d31a0503eaf1e0029880` merges into the native catalog, retains dashboard pins, fingerprints installed renderer helpers, locks the catalog and verifies/rolls back without replacing dashboard application code. September24 is live_verified with686 retained articles. Public JSON must be0644; a0600 dashboard-written catalog caused403-to-home redirects and broken native search. Publisher repairs public output permissions; dashboard-writer followup is recorded in TW-TASK-0008. Legacy Cloudflare email-link transformations are reversed only for exact archived HTML verification. Home/search remain noindex. This supersedes the recovery-only target descriptions below; production remains unchanged. See TW-TASK-0005 for completion, preserved rollback attempts and receipts.

**SMN fresh subscription edition, September 21, 2026.** Six same-day production selections are now published to the existing .176 Dev review site through saved ChatGPT subscription Astra xhigh, with fresh primary-source context and unchanged TradeWave mathematics. Normal SMN presentation is restored. Main/live source c2beec6778c89592a8e12d3ac4266d8aea9e0ac0. Root now serves cumulative production wire home with 30 retained articles after the September22 edition and full archive search; see TW-BUG-0011. The existing daily 07:00 America/New_York heartbeat uses the clean local smn-resilience-integrated-20260922 checkout and immutable smn-review-20260921/production-style edition roots. Windows awake/Codex availability required; no paid API fallback or production writes. [TW-TASK-0005](tasks/TW-TASK-0005.md) records the live proof, schedule and private replay evidence.
...
**Phone rotation audit, September 15, 2026, frontend `fce41885`:** the caught
`MobileLayoutL` reset-effect/Swiper `this.params.speed` failure and lifecycle
