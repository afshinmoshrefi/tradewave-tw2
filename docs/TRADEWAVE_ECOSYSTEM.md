...
**SMN visual proof representation:** publication compares SHA256 strings exactly against lowercase Python hexdigests. Write visual-checks.json hash fields as lowercase (PowerShell Get-FileHash otherwise emits uppercase). Equal file bytes with uppercase proof strings still fail the current packaging gate; preserve failed attempts rather than silently retrying. TW-TASK-0005 September22 run records recovery state.
