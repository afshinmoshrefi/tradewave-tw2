...
def test_named_symbol_read_card_supplies_missing_load_action():
    actions = []
    cards = {
        "ITW": {
            "symbol": "ITW",
            "market": "2",
            "setup": {"entry_date": "2025-08-02", "hold_days": 208},
            "stats": {"years": 16},
        }
    }

    tara_gateway._enforce_named_symbol_action(actions, cards, "ITW", 2026)
...
def test_named_symbol_action_inherits_current_lookback_and_caps_to_available_record():
    actions = [
        {
            "type": "set_view",
            "spec": {
    ...
    cards = {
        "ITW": {
            "symbol": "ITW",
            "headline": "ITW long - Won 12/12 years, avg +8.0%.",
            "stats": {"years": "16"},
        }
    }

    tara_gateway._enforce_named_symbol_action(
        actions, cards, "ITW", 2026, preferred_years=16
    )

    assert actions[0]["spec"]["entry_date"] == "2026-08-02"
    assert actions[0]["spec"]["years"] == 12
...
def test_named_symbol_inherited_lookback_steps_down_to_target_metadata(monkeypatch):
    ...
    monkeypatch.setattr(
        tara_gateway,
        "_symbol_max_available_years",
        lambda market, symbol, token: 12,
    )
...
def test_system_prompt_forces_named_symbol_over_loaded_symbol():
    ...
    blocks = chatbot_module.build_system_prompt(
        ...
        screen_context={},
        user_message=(
            "What does TWR reveal about this ITW pattern that Sharpe misses?"
        ),
    )
    ...
    assert "Call analyze_symbol for ITW" in prompt
    assert "Then call update_view with the verified ITW setup" in prompt
    assert "current 2026 occurrence" in prompt
