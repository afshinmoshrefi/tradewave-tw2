...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("please pull up the stock price chart", "price_chart", "Price Chart"),
    ],
)
...
def test_explicit_loaded_pattern_analysis_is_compact_path_aware_and_non_advisory():
    ...
    assert "recent record: 2 of 5 profitable versus 12 of the prior 12" in reply
    assert "this small slice does not establish lasting deterioration" in reply
...
def test_near_perfect_record_does_not_claim_a_stable_payoff_from_one_loss():
    ...
    reply = build_pattern_analysis_reply(
        "Analyze", wave, {}, current_year=2026
    )

    assert facts["only_losing_year"] == 2021
    assert facts["only_losing_mfe_pct"] == 11.97
    assert facts["only_losing_return_pct"] == -2.09
    assert "positive in all but one completed observation" in reply
    assert "Sharpe 1.64 (final returns); TWR 2.31 (MFE)" in reply
    assert "with only one losing observation" in reply
    assert "payoff ratio is not stable enough to interpret" in reply
    assert "only losing observation (2021) still reached +11.97% MFE" in reply
    assert "before finishing -2.09%" in reply
    assert "14.06-percentage-point giveback from its best move" in reply
    assert "Win frequency and payoff were favorable" not in reply

    prompt_tail = "\n".join(verified_context_lines(wave, {}, current_year=2026))
    assert "TWR) 2.31 applies the Sharpe-style return-to-dispersion calculation" in prompt_tail
    assert "each year's MFE instead of its final return" in prompt_tail
    assert "only losing observation, 2021, reached +11.97% MFE" in prompt_tail
    assert "before finishing -2.09%" in prompt_tail
    assert "14.06-percentage-point giveback" in prompt_tail


def test_long_pattern_ai_checkpoints_use_positive_calibrated_copy():
    wave = _analysis_context([2.0, -1.0] * 8, years="16")
    wave["days_out"] = "208"
    wave["ai_analysis"] = {
        "status": "available",
        "mode": "checkpoints",
        "full_pattern_calendar_days": 208,
        "horizons": [
            {
                "calendar_days": 30,
                "ai_score": 85,
                "win_probability": 0.85,
                "predicted_return_pct": 3.8,
            },
            {
                "calendar_days": 60,
                "ai_score": 77,
    ...
    assert "first 30, 60, and 90 calendar days are as follows" in reply
    assert "&bull; <b>30 days:</b> 85% AI Win Probability; predicted return +3.8%" in reply
    assert "&bull; <b>60 days:</b> 77% AI Win Probability; predicted return +4.2%" in reply
    assert "&bull; <b>90 days:</b> 61% AI Win Probability; predicted return +1.7%" in reply
    assert "highest AI Win Probability appears over 30 days" in reply
    assert "highest predicted return appears over 60 days" in reply
    assert "historical analysis above describes the complete 208-day pattern" in reply
...
def test_explicit_named_symbol_overrides_a_different_loaded_pattern():
    wave = _analysis_context([2.0, -1.0] * 5)
    wave["symbol"] = "TDG"
    message = "How good is this ITW pattern?"

    assert explicit_pattern_symbol(message) == "ITW"
    assert explicit_pattern_symbol("What does TWR reveal about this ITW pattern?") == "ITW"
    assert explicit_pattern_symbol("How good is this pattern?") is None
    assert build_deterministic_reply(message, wave, {}, current_year=2026) is None
...
def test_verified_prompt_facts_state_short_semantics_positively():
    ...
    assert "each year's MFE instead of its final return" in prompt_tail
    assert "Sharpe 0.82 uses the final returns" in prompt_tail
    assert "Entry-relative path: median MFE +4.50%; median MAE -1.00%" in prompt_tail
    assert "Losing-finish path: the losing observations still reached median MFE +0.50%" in prompt_tail
    assert "Recent slice: 2 of 5 profitable versus 12 of 12 prior" in prompt_tail
    assert "too small by itself to establish lasting deterioration" in prompt_tail
...
def test_compact_named_symbol_question_overrides_the_loaded_symbol():
    ...
    assert explicit_pattern_symbol("show me AJG") == "AJG"
