...
"Sell in May and go away" is the kind of market aphorism that sounds too neat to be true. The advice has been recited in London brokerage offices for the better part of two centuries, which is suspicious in itself - folk wisdom in finance usually decays under serious testing. The interesting thing about the Halloween effect is that it largely has not. Twenty-plus years after Sven Bouman and Ben Jacobsen put real numbers behind the saying, the gap between winter-half and summer-half stock returns is still showing up in fresh data, and researchers still cannot fully agree on why.

This article walks through what we know: the 2002 finding, the most serious critiques, the out-of-sample replications, and the candidate mechanisms. Then it gets to the part most readers want, which is what to do with it as a working investor.

The short version: the effect is real, it is small but persistent, and the worst thing you could do with it is treat it as a binary trading rule that pulls you to cash every spring. The right way to use it is as a small tilt within a strategy that has other reasons to exist.
...
To their credit, the authors did not just report the result and walk away. They went through the obvious alternative explanations one by one. Was it driven by sector composition? No, the effect held across sectors. Was it just the January effect in disguise? No, it survived removing January. Was it cross-correlation among markets that all happened to do well in winter? Partially, but the magnitude was too large for that. Was it data mining? They argued no, since the saying predated the data they were testing.
...
This is exactly the kind of critique that should make any honest researcher sweat. Two outliers driving a 30-year result is a real concern, and Maberly and Pierce were right to flag it.

But two things about the critique cut against its force. First, it applies specifically to the U.S. The Bouman-Jacobsen paper's main result was the international one - 36 out of 37 markets. Even if you grant that the U.S. result is outlier-driven, that does not explain why the pattern shows up almost everywhere else. Second, Russell Witte's 2010 follow-up in the *International Review of Financial Analysis* argued that other outliers cut the other way, and that adjusting for the full set of large monthly moves leaves the strategy looking similar to the unadjusted version. The "outlier" argument is not as one-sided as the original critique implied.
...
This is the body of evidence that has convinced most academics the effect is a real statistical regularity rather than a data-mining artifact. You can argue about the mechanism. You can argue about whether it is exploitable after costs. You cannot easily argue that the pattern does not exist in the data.
...
None of the three is definitive. They are also not mutually exclusive. The honest summary is that a 5 to 10 percentage point annual gap is too large to be fully explained by any single mechanism the literature has put forward, and the question is open.
...
The reasonable use of the Halloween effect, in our view, is as a small tilt rather than a binary rule. If your strategy already has a mechanical reason to rebalance, lean modestly toward higher equity weight in the Nov-Apr window. If you run a tactical overlay on a buy-and-hold core, a modest seasonal tilt is one of several signals you might combine with valuation, momentum, and macro factors. What you should not do is convert a 5 percentage point statistical regularity into a 100% on/off allocation.

The other thing worth doing is testing it on your own universe. The aggregate result holds across countries and decades. That does not mean it holds for the specific tickers you trade, on the specific timeframe you care about, after the specific costs you face. Anomalies that look beautiful in aggregate often look much messier on individual symbols, and the only way to find out is to run the data yourself.
...
The most useful frame is this: the Halloween effect is one of the cleanest examples in the seasonality literature of an anomaly that should have been arbitraged away, and largely has not been. That tells you something about the limits of arbitrage in retail markets, and it tells you that "everyone knows about this" is not the same thing as "everyone trades on this." Most of the people who know about the Halloween effect are not in a position to act on it consistently after costs. The gap has narrowed somewhat over the decades, but it is still there, and there is no clean reason to expect it to vanish in the next ten years.
