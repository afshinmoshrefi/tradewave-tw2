...
A backtested strategy with a Sharpe ratio of 2.0 sounds impressive. In a vacuum, it implies returns roughly twice the volatility, the kind of risk-adjusted performance that justifies a hedge-fund letter. The uncomfortable reality is that on a single random pull from US equity data, with a few hundred strategy variations tried in the background, a Sharpe of 2.0 is approximately what flipping coins would give you. The strategy is not the signal. The search is.
...
<div class="article-tw-callout"><strong>Practical implication.</strong> The good news for self-directed investors: most of the discipline these papers prescribe (out-of-sample testing, cross-validation across cycles, multiple-testing awareness) is doable in any analysis tool that lets you filter by year ranges or cycle phases - including TradeWave's Wave Viewer (/app/). The hard part is remembering to be skeptical when a backtest looks too good.</div>

## Putting the pitfalls together

A useful mental model: each of the five pitfalls inflates apparent performance by some amount, and the inflations compound. A backtest that suffers from all five (multiple testing across hundreds of variations, a survivor-only universe, retroactively adjusted prices, in-sample parameter tuning, and a sample drawn from a single bull regime) can plausibly turn a true Sharpe of zero into a reported Sharpe of 2 or 3. This is not malicious; it is the default behaviour of unreflective backtesting.

The literature converges on a few practical heuristics for self-defense. First, write down your hypothesis before you look at the data, in plain English, and explain why it should work economically. Second, choose your validation scheme before you compute any returns: walk-forward windows, embargo periods, the metric you will use to judge success. Third, count your trials and apply a multiple-testing correction; if you cannot apply the deflated Sharpe formula directly, at minimum require t > 3 rather than t > 2. Fourth, replicate published results with point-in-time data and a survivorship-bias-free universe before believing them. Fifth, look at the most recent five years of data with extra suspicion: a strategy that worked from 1990 to 2018 but stalled from 2019 to today is probably a dying strategy, not a buying opportunity.

## Practical takeaway

A strategy with Sharpe 2.0 in-sample on a single random pull is roughly equivalent to flipping coins, given how easy it is to find such results by accident. Real edge requires either economically motivated hypotheses tested honestly, or sample sizes large enough that the statistical bar is hard to clear by chance. Most retail backtest software lets you fall into all five traps simultaneously, and most retail "I built a Sharpe-3 strategy" claims fall into at least three.

The discipline that separates working quant practice from exhibitionist backtesting is not exotic. It is the willingness to count your trials, hold out genuine out-of-sample data, use point-in-time prices, test across regimes, and accept that most "discoveries" do not survive contact with the future.
