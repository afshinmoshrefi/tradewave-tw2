#!/usr/bin/env python3
"""TradeWave Learn generator (platform guides / how-tos).

Reads markdown articles from /home/flask/site/content/learn/<slug>.md,
renders them with the shared TW2 header into static HTML at
/var/www/tradewave/learn/.

Parallel to generate_insights.py - same machinery, separate content area.
Learn = product walkthroughs (Wave Viewer, Stats Table, ...). Insights =
...
Article frontmatter (YAML between --- fences):
    ...
    date:            ISO YYYY-MM-DD.
    author:          Default "TradeWave Research" (Michael Sacchitello for v1).
    reading_minutes: Estimate.
    order:           Curated sort key (small=top of index). Falls back to date desc.
...
CONTENT_DIR   = SITE_ROOT / 'content' / 'learn'
TEMPLATES_DIR = SITE_ROOT / 'templates'
HEADER_PATH   = TEMPLATES_DIR / '_tw_header.html'
OUTPUT_DIR    = Path('/var/www/tradewave/learn')
...
def load_articles() -> List[Dict[str, Any]]:
    ...
    # Articles without 'order' fall to the end, then by date desc.
    arts.sort(key=lambda a: (a.get('order', 9999), -datetime.strptime(a['date_iso'], '%Y-%m-%d').toordinal()))
    return arts
...
def update_sitemap(articles: List[Dict[str, Any]]) -> None:
    ...
    rebuilds it from disk on every run (scans learn/*.html itself). This append-only
...
def main() -> int:
    ...
    # Copy article images from repo (site/content/learn/images/) to serve dir
    # (/var/www/tradewave/learn/images/). Insights generates charts from data on each
    # box; Learn uses static screenshots that ship in the repo, so just mirror them.
    src_img = CONTENT_DIR / 'images'
    if src_img.is_dir():
        dst_img = OUTPUT_DIR / 'images'
        dst_img.mkdir(parents=True, exist_ok=True)
        import shutil as _sh
        for f in src_img.iterdir():
            if f.is_file():
                _sh.copy2(f, dst_img / f.name)
    ...
    article_tmpl = env.get_template('learn_article.html')
    index_tmpl   = env.get_template('learn_index.html')
    ...
    print(f'TradeWave Learn generator')
