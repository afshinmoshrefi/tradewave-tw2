...
# Redis executes each script as one indivisible operation.  A GET followed by a
# client-side INCRBY is not sufficient here: concurrent MCP requests can all see
# the same remaining allowance and overspend it before any increment lands.
_CONSUME_LUA = """
local used = tonumber(redis.call('GET', KEYS[1]) or '0')
if used == nil or used < 0 then
  return redis.error_reply('invalid ML quota counter')
end
local limit = tonumber(ARGV[1])
local requested = tonumber(ARGV[2])
local ttl = tonumber(ARGV[3])
if limit == nil or requested == nil or ttl == nil or requested <= 0 then
  return redis.error_reply('invalid ML quota arguments')
end
...
if amount == nil or ttl == nil or amount <= 0 then
  return redis.error_reply('invalid ML quota arguments')
end
...
def consume(cust, n):
    ...
    k = _key(cust["user_id"])
    try:
        # One server-side transaction both decides and records the grant.  This
        # preserves the entitlement even when many async MCP calls arrive at once.
...
def refund(cust, n):
    ...
    k = _key(cust["user_id"])
    try:
        # Keep the floor-zero refund atomic with concurrent reservations.  A
        # client-side read/set can otherwise erase a reservation that lands in
